...
import hashlib
import os
import traceback---------------------------Configuration---------------------------APP_TITLE = "COSNA School Management System"
DB_PATH = "cosna_school.db"
REGISTRATION_FEE = 50000.0
SIMILARITY_THRESHOLD = 0.82
LOGO_FILENAME = "school_badge.png"  # place uploaded badge here
PAGE_LAYOUT = "wide"st.set_page_config(page_title=APP_TITLE, layout=PAGE_LAYOUT, initial_sidebar_state="expanded")
st.title(APP_TITLE)
st.markdown("Students • Uniforms • Finances • Reports")---------------------------Utilities---------------------------def get_db_connection():
    conn = sqlite3.connect(DB_PATH, check_same_thread=False)
    conn.row_factory = sqlite3.Row
    return conndef normalize_text(s: str):
    if s is None:
        return ""
    return " ".join(s.strip().lower().split())def similar(a: str, b: str):
    if not a or not b:
        return 0.0
    return difflib.SequenceMatcher(None, normalize_text(a), normalize_text(b)).ratio()def is_near_duplicate(candidate: str, existing_list, threshold=SIMILARITY_THRESHOLD):
    candidate_n = normalize_text(candidate)
    for ex in existing_list:
        if similar(candidate_n, ex) >= threshold:
            return True, ex
    return False, Nonedef hash_password(password: str, salt: str = None):
    if salt is None:
        salt = os.urandom(16).hex()
    hashed = hashlib.sha256((salt + password).encode('utf-8')).hexdigest()
    return f"{salt}${hashed}"def verify_password(stored: str, provided: str):
    try:
        salt, _ = stored.split('$', 1)
    except Exception:
        return False
    return hash_password(provided, salt) == storeddef generate_code(prefix="RCPT"):
    day = datetime.now().strftime("%d")
    random_chars = ''.join(random.choices(string.ascii_uppercase + string.digits, k=2))
    return f"{prefix}-{day}{random_chars}"def generate_receipt_number(): return generate_code("RCPT")
def generate_invoice_number(): return generate_code("INV")
def generate_voucher_number(): return generate_code("VCH")def safe_rerun():
    try:
        ...
        try:
            ...
            pass---------------------------DB migration helpers---------------------------def table_has_column(conn, table_name, column_name):
    cur = conn.cursor()
    cur.execute(f"PRAGMA table_info({table_name})")
    cols = [r[1] for r in cur.fetchall()]
    return column_name in colsdef safe_alter_add_column(conn, table, column_def):
    ...
    try:
        ...
        return False
    return False---------------------------Initialize DB and seed---------------------------def initialize_database():
    conn = get_db_connection()
    cursor = conn.cursor()# Core tables
cursor.execute('''
...
conn.close()initialize_database()---------------------------Audit logging---------------------------def log_action(action, details="", performed_by="system"):
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        ...
        pass---------------------------Authentication---------------------------def get_user(username):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT * FROM users WHERE username = ?", (username,))
    row = cur.fetchone()
    conn.close()
    return rowif 'user' not in st.session_state:
    st.session_state.user = None---------------------------Logo handling---------------------------def logo_exists():
    return os.path.exists(LOGO_FILENAME)def save_uploaded_logo(uploaded_file):
    try:
        with open(LOGO_FILENAME, "wb") as f:
            f.write(uploaded_file.getbuffer())
        return True
    except Exception:
        return False---------------------------Export helpers (Excel & PDF landscape)---------------------------def df_to_excel_bytes(df: pd.DataFrame, sheet_name="Sheet1"):
    buf = BytesIO()
    with pd.ExcelWriter(buf, engine='xlsxwriter') as writer:
        df.to_excel(writer, sheet_name=sheet_name, index=False)
    buf.seek(0)
    return bufdef dataframe_to_pdf_bytes_landscape(df: pd.DataFrame, title="Report", logo_path=None):
    buf = BytesIO()
    c = canvas.Canvas(buf, pagesize=landscape(letter))
    width, height = landscape(letter)# Draw logo if available
y_top = height - 30
title_x = 40
draw_h = 0
...
c.save()
buf.seek(0)
return bufdef download_options(df: pd.DataFrame, filename_base="report", title="Report"):
    ...
    with col2:
        ...
        st.download_button("Download PDF (Landscape)", pdf_buf, f"{filename_base}.pdf", "application/pdf")---------------------------Role-based access helper---------------------------def require_role(allowed_roles):
    ...
    if user.get('role') not in allowed_roles:
        ...
        st.stop()---------------------------Login page (isolated)---------------------------def show_login_page():
    st.markdown("### Login")
            ...
            if submit:
                ...
                if not username or not password:
                    ...
                    if user and verify_password(user["password_hash"], password):
                        ...
                        st.error("Invalid credentials")if not st.session_state.user:
    show_login_page()
    st.stop()---------------------------Sidebar after login---------------------------with st.sidebar:
    if logo_exists():
        try:
    ...
    if st.button("Logout"):
        ...
        safe_rerun()---------------------------Global Term Selector in Sidebar---------------------------conn_global = get_db_connection()
...
""", conn_global)
conn_global.close()if terms_df.empty:
    ...
    st.session_state['selected_term_label'] = term_labels[selected_index]---------------------------Main navigation---------------------------page = st.sidebar.radio("Menu", ["Dashboard", "Students", "Uniforms", "Finances", "Financial Report", "Fee Management", "Cashbook", "Audit Log"])Helper to get term filter SQL and paramsdef get_term_filter():
    if st.session_state.get('selected_term_id'):
        return " AND term_id = ?", [st.session_state['selected_term_id']]
    return "", []---------------------------Dashboard---------------------------if page == "Dashboard":
    conn = get_db_connection()
    st.header(" Financial Overview")
    col1, col2, col3, col4 = st.columns(4)term_sql, term_p = get_term_filter()
...
try:
    ...
    st.info("No monthly data available")
conn.close()---------------------------Students---------------------------elif page == "Students":
    st.header("Students")
    tab_view, tab_add, tab_fees, tab_classes = st.tabs(["View & Export", "Add Student", "Student Fees", "Manage Classes"])with tab_view:
    conn = get_db_connection()
    try:
...
with tab_classes:
    ...
    conn.close()---------------------------Uniforms---------------------------elif page == "Uniforms":
    st.header("Uniforms – Inventory & Sales")
    conn = get_db_connection()def get_inventory_df():
...
conn.close()---------------------------Finances---------------------------elif page == "Finances":
    user_role = st.session_state.user.get('role')
    st.header("Finances")
    tab_inc, tab_exp, tab_reports = st.tabs(["Record Income", "Record Expense", "View Transactions"])term_sql, term_p = get_term_filter()
...
with tab_reports:
    ...
    conn.close()---------------------------Financial Report---------------------------elif page == "Financial Report":
            ...
            try:
                ...
                if report_type == "Income vs Expense (date range)":
                    ...
                    if df_inc.empty and df_exp.empty:
                        ...
                        download_options(combined, f"financial_{start_date}{end_date}", "Income vs Expense Report")
                    ...
                    if df.empty:
                        ...
                        st.dataframe(df, use_container_width=True)
                        download_options(df, f"by_category{cat}", f"By Category - {cat}")
                    ...
                    if students.empty:
                        ...
                        if df_inv.empty:
                            ...
                            download_options(df_inv, f"student_{sid}invoices", f"Invoices for Student {sid}")
                        st.subheader("Payments")
                        if df_pay.empty:
                            st.info("No payments for this student")
                        else:
                            st.dataframe(df_pay, use_container_width=True)
                            download_options(df_pay, f"student{sid}_payments", f"Payments for Student {sid}")
            except Exception as e:
                st.error(f"Error generating report: {e}")
    conn.close()---------------------------Cashbook---------------------------elif page == "Cashbook":
    ...
    conn.close()---------------------------Audit Log---------------------------elif page == "Audit Log":
    ...
    conn.close()---------------------------Fee Management---------------------------elif page == "Fee Management":
    require_role(["Admin", "Accountant"])
    st.header("Fee Management")
    conn = get_db_connection()st.subheader("Configure Academic Terms")

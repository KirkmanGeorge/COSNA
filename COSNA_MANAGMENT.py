...
def get_db_connection():
    # Connect to Supabase PostgreSQL using provided secrets
    # Assuming secrets are in st.secrets or os.environ; adjust if needed
    ...
    # No row_factory needed; we'll use dict cursors where necessary
    return conn
...
def initialize_database():
    ...
    cursor = conn.cursor()
    # Core tables - Adapted to PostgreSQL syntax (SERIAL for AUTOINCREMENT, TIMESTAMP for DATETIME)
    cursor.execute('''
    ...
    ''')
    conn.commit()
    # Safe migrations - Using PostgreSQL syntax
    ...
    # Backfill normalized fields - Adapted to use %s placeholders
    try:
        cursor.execute("SELECT id, category, normalized_category FROM uniform_categories")
        rows = cursor.fetchall()
        for r in rows:
            if (r[2] is None or r[2] == "") and r[1]:
                cursor.execute("UPDATE uniform_categories SET normalized_category = %s WHERE id = %s", (normalize_text(r[1]), r[0]))
        conn.commit()
    except:
        pass
    try:
        cursor.execute("SELECT id, name, normalized_name FROM students")
        rows = cursor.fetchall()
        for r in rows:
            if (r[2] is None or r[2] == "") and r[1]:
                cursor.execute("UPDATE students SET normalized_name = %s WHERE id = %s", (normalize_text(r[1]), r[0]))
        conn.commit()
    except:
        pass
    try:
        cursor.execute("SELECT id, name, normalized_name FROM staff")
        rows = cursor.fetchall()
        for r in rows:
            if (r[2] is None or r[2] == "") and r[1]:
                cursor.execute("UPDATE staff SET normalized_name = %s WHERE id = %s", (normalize_text(r[1]), r[0]))
        conn.commit()
    except:
        pass
    # Ensure uniforms rows
    try:
        cursor.execute("SELECT id FROM uniform_categories")
        rows = cursor.fetchall()
        for r in rows:
            cat_id = r[0]
            cursor.execute("SELECT id FROM uniforms WHERE category_id = %s", (cat_id,))
            u = cursor.fetchone()
            if not u:
                cursor.execute("INSERT INTO uniforms (category_id, stock, unit_price) VALUES (%s, 0, 0.0)", (cat_id,))
        conn.commit()
    except:
        pass
    # Seed default admin
    try:
        ...
        if cursor.fetchone()[0] == 0:
            ...
            cursor.execute(
                "INSERT INTO users (username, password_hash, role, full_name) VALUES (%s, %s, %s, %s) ON CONFLICT DO NOTHING",
    ...
    try:
        for name, gender, shared in uniform_seeds:
            ...
            cursor.execute("SELECT id FROM uniform_categories WHERE normalized_category = %s OR category = %s", (nname, name))
            row = cursor.fetchone()
            if not row:
                cursor.execute(
                    "INSERT INTO uniform_categories (category, normalized_category, gender, is_shared) VALUES (%s, %s, %s, %s) RETURNING id",
    ...
    try:
        for cat, cat_type in expense_seeds:
            cursor.execute("SELECT id FROM expense_categories WHERE name = %s", (cat,))
            if not cursor.fetchone():
                cursor.execute("INSERT INTO expense_categories (name, category_type) VALUES (%s, %s)", (cat, cat_type))
                conn.commit()
    except:
        pass
    conn.close()
...
def get_user(username):
    ...
    if row:
        # Convert tuple to dict for easier access (same keys as original sqlite Row)
...
def get_terms():
    ...
    df = pd.read_sql(
        ...
        "FROM terms ORDER BY academic_year DESC, term DESC",
        conn
    )
    conn.close()
    return df
...
if page == "Dashboard":
    ...
    with colA:
        ...
        try:
            df_inc = pd.read_sql(
                f"SELECT date as Date, receipt_number as \"Receipt No\", amount as Amount, "
                f"source as Source, payment_method as \"Payment Method\", payer as Payer, "
                f"description as Description FROM incomes {inc_where} "
...
# ────────────────────────────────────────────────
    ...
    with tab_view:
        ...
        try:
            query = """
                SELECT s.id as ID, s.name as Name, s.age as Age, s.enrollment_date as "Enrollment Date",
    ...
    with tab_edit:
        ...
        students = pd.read_sql(
            "SELECT s.id, s.name, c.name as class_name FROM students s LEFT JOIN classes c ON s.class_id = c.id ORDER BY s.name",
            conn
        )
        if students.empty:
            ...
            with st.form("edit_student_form"):
                ...
                with col2:
                    cls_df = pd.read_sql("SELECT id, name FROM classes ORDER BY name", conn)
                    cls_options = cls_df["name"].tolist() if not cls_df.empty else []
                    current_cls_name = pd.read_sql(
                        "SELECT name FROM classes WHERE id = %s", conn, params=(student_row['class_id'],)
                    ).iloc[0]['name'] if student_row['class_id'] else "-- No class --"
                    ...
                    student_type = st.radio("Student Type", ["New", "Returning"],
                    ...
                    if not name:
                        ...
                        try:
                            ...
                            if dup:
                                ...
                                cur.execute("""
                                    UPDATE students
                                    SET name = %s, normalized_name = %s, age = %s, enrollment_date = %s, class_id = %s, student_type = %s
                                    WHERE id = %s
        ...
        if students.empty:
            ...
            try:
                payments = pd.read_sql(
                    f"SELECT p.payment_date, p.amount, p.payment_method, p.receipt_number, "
                    f"p.reference_number, p.notes FROM payments p "
                    f"JOIN invoices i ON p.invoice_id = i.id {inv_where} ORDER BY p.payment_date DESC",
            ...
            if incomes.empty:
                ...
                with st.form("edit_income_form"):
                    ...
                    category = st.selectbox(
                        ...
                        index=categories["name"].tolist().index(current_cat_name) + 1 if current_cat_name != "-- Select --" else 0
                    )
            ...
            try:
                if report_type == "Income vs Expense (date range)":
                    df_inc = pd.read_sql(
                        ...
                        "description as Description FROM incomes "
    ...
    with tab_generate:
        ...
        if students.empty:
            ...
            if fee_options.empty:
                ...
                idx = fee_options.apply(lambda x: f"{x['academic_year']} - {x['term']} (USh {x['total_fee']:,.0f})", axis=1) == chosen
                fee_row = fee_options.loc[idx].iloc[0]
